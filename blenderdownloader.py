...
def main(argv):
	...
	try:
		blender_html = BS(urllib2.urlopen(urllib2.Request('http://www.blender.org')).read())
	...
	def genlink(version=blenderCurVer.replace("Blender ",""), platform="windows64"):
		...
		try:
			...
			return final
		return final	
	def download(link="http://download.blender.org/release/Blender2.70/blender-2.70-windows64.zip"):
	...
	def extract(zipped="blender-2.70-windows64.zip"):
		...
		print "\nExtracting: %s filecount: %s" % (zipped, fileCount)
		count = 0
